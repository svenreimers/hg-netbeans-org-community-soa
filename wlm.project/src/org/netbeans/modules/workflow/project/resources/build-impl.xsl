  ...
  - cleanup
    ...
    ======================
            ...
            <target name="init-project">
                ...
                <!-- <property name="module.install.dir" value="${{cluster}}/modules"/> -->
            </target>
            <target name="do-init">
...
<!-- //B20050104                 <fail unless="build.classes.excludes">Must set build.classes.excludes</fail> -->
            ...
            <target name="init-taskdefs-1">
                <path id="ant.task.classpath.models">
                    ...
                    <pathelement path="${{esb.netbeans.platform}}/../soa/modules/ext/anttask.jar"/>
                    ...
                    <pathelement path="${{esb.netbeans.platform}}/../soa/modules/ext/chiba-1.0.0.jar" />
                    <pathelement path="${{esb.netbeans.platform}}/../xml/modules/ext/jxpath-1.2.jar"/>
                ...
                <path id="generate.jbi">
                    ...
                    <pathelement path="${{esb.netbeans.platform}}/../soa/modules/ext/anttask.jar"/>
                    <pathelement path="${{esb.netbeans.platform}}/../soa/modules/ext/wfse_def.jar-1.0.0.jar"/>
                    <pathelement path="${{esb.netbeans.platform}}/../ide/modules/org-netbeans-modules-xml-xam.jar"/>
                    <pathelement path="${{esb.netbeans.platform}}/../ide/modules/org-netbeans-modules-xml-schema-model.jar"/>
                    <pathelement path="${{esb.netbeans.platform}}/../ide/modules/org-netbeans-modules-xml-wsdl-model.jar"/>
                    <pathelement path="${{esb.netbeans.platform}}/../soa/modules/ext/wsdl4j.jar"/>
                    <pathelement path="${{esb.netbeans.platform}}/../soa/modules/ext/qname.jar"/>
                    <pathelement path="${{esb.netbeans.platform}}/../soa/modules/ext/xbean.jar"/>
                    <pathelement path="${{esb.netbeans.platform}}/../soa/modules/ext/xbean_xpath.jar"/>
                    <pathelement path="${{esb.netbeans.platform}}/../soa/modules/ext/resolver.jar"/>
                    <pathelement path="${{esb.netbeans.platform}}/../soa/modules/ext/jsr173_1.0_api.jar"/>
                    <pathelement path="${{esb.netbeans.platform}}/../soa/modules/ext/xmlpublic.jar"/>
                    <pathelement path="${{esb.netbeans.platform}}/../soa/modules/ext/chiba-1.0.0.jar"/>
                    <pathelement path="${{esb.netbeans.platform}}/../xml/modules/ext/jxpath-1.2.jar"/>
                ...
                <manifest file="${{build.classes.dir}}/META-INF/MANIFEST.MF" mode="update">
                    <attribute>
                        ...
                        <xsl:attribute name="value"><xsl:for-each select="//ejb:included-library">
                                ...
                                <xsl:value-of select="$base.prop.name"/></xsl:for-each></xsl:attribute>
                    </attribute>
                </manifest>
            </target>
...
<!--                <move file="${{build.dir}}/org/netbeans/modules/workflow/project/resources/TaskStaticTemplate.wsdl" tofile="${{basedir}}/${{build.dir}}/${{projectName}}_wf_client_temp.wsdl"/> -->
...
<!--                <replace file="${{basedir}}/${{build.dir}}/${{projectName}}_wf_client_temp.wsdl" token="DEFINITON_TARGETNAMESPACE" value="http://jbi.com.sun/wfse/wsdl/${{projectName}}_wf_client"/> 
                <WFGenClientWSDL sourcedirectory="${{basedir}}/${{src.dir}}" builddirectory="${{basedir}}/${{build.dir}}" clientwsdl="${{basedir}}/${{build.dir}}/${{projectName}}_wf_client_temp.wsdl" projectname="${{projectName}}" />
        <delete file="${{basedir}}/${{build.dir}}/${{projectName}}_wf_client_temp.wsdl"/> -->
            ...
            <target name="dist_se">
                ...
                <jar compress="${{jar.compress}}" jarfile="${{build.dir}}/SEDeployment.jar">
                    ...
                    <!-- since the fileset include (above) is including jbi.xml also, the following 
                    is masked to prevent duplicate packaging of jbi.xml -->
...
-->
    ...
    =================
        ...
        <generate-deployment-path
            jarFileClasspath="${{module.install.dir}}/com-sun-jbi-ui-devtool-projects-xsltpro.jar"
